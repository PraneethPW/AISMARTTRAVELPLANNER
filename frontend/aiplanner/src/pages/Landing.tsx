...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <section id="features" className="bg-slate-900 px-4 py-20 sm:px-6 lg:px-8">
            ...
            {pages.map(([key, page]) => {
              ...
              return (
                <Link
                  key={key}
                  to={`/${key}`}
                  className="group rounded-3xl border border-white/10 bg-slate-950/55 p-5 transition hover:-translate-y-1 hover:border-cyan-300/45 hover:bg-slate-950"
                >
      ...
      <section id="reviews" className="border-y border-white/10 bg-slate-950 px-4 py-20 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
            <div>
              <p className="text-sm font-black uppercase tracking-[0.28em] text-amber-300">Testimonials</p>
              <h2 className="mt-4 text-3xl font-black sm:text-5xl">Built for travellers who need clarity fast.</h2>
            </div>
            <Link to="/register" className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-black text-slate-950">
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            {testimonials.map((item) => (
              <figure key={item.name} className="rounded-3xl border border-white/10 bg-white/[0.06] p-6">

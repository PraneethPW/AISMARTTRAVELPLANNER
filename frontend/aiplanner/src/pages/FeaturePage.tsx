...
const EmptyState = () => (
  <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center">
    ...
    <Link to="/planner" className="mt-5 inline-flex items-center gap-2 rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white">
...
const FoodCard = ({ food }: { food: FoodOption }) => (
  <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <h3 className="mt-4 text-lg font-black">{food.name}</h3>
...
const TimelineRow = ({ item }: { item: TimelineItem }) => (
  <div className="grid gap-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:grid-cols-[80px_1fr_120px]">
    <div>
    ...
    <DashboardLayout>
      <div className="space-y-7">
        <section className="overflow-hidden rounded-[2rem] bg-slate-950 text-white shadow-2xl">
          <div className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[1fr_0.65fr]">
            <div>
              <p className="text-sm font-black uppercase tracking-[0.28em] text-cyan-300">{page.eyebrow}</p>
              <h1 className="mt-4 text-3xl font-black tracking-tight sm:text-5xl">{page.title}</h1>
            ...
            <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.06] p-5">
              ...
              <p className="mt-2 text-2xl font-black">{trip ? trip.destination : "No plan"}</p>
        ...
        {plan && type === "stays" && (
          <section className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{(plan.stays?.hotels || []).map((stay) => <StayCard key={stay.name} stay={stay} label="Hotel" />)}</div>
            <SectionTitle title="Hostels" />
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{(plan.stays?.hostels || []).map((stay) => <StayCard key={stay.name} stay={stay} label="Hostel" />)}</div>
            <SectionTitle title="PG and long-stay options" />
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{(plan.stays?.pgs || []).map((stay) => <StayCard key={stay.name} stay={stay} label="PG" />)}</div>
          </section>
        )}
        ...
        {plan && type === "food" && (
          <section>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{(plan.food || []).map((food) => <FoodCard key={food.name} food={food} />)}</div>
          </section>
        )}
        ...
        {plan && type === "places" && (
          <section>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{(plan.famous_places || []).map((place) => <PlaceCard key={place.name} place={place} />)}</div>
          </section>
        )}
